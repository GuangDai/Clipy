/// M1.4/M1 + X.3 + J.2 + byte-preserving titles — V1 → V2 → V3 → V4 → V5.
...
/// V4 → V5 adds titleUTF8 with an empty default. The Authority's subsequent
/// projection rebuild derives its bytes from unchanged content lineage,
/// never from the potentially lossy legacy title String (05 §13/§15).
import SwiftData
...
internal enum HistoryMigrationPlan: SchemaMigrationPlan {
    ...
    /// (`V2-roadmap` J.2/J.3; DC-25), then V5's byte-preserving title column.
    /// Shipped schema fields are never edited.
    internal static var schemas: [any VersionedSchema.Type] {
        [
            ...
            HistorySchemaV5.self,
        ]
    }

    /// Four ship-ordered stages: the custom `V1 → V2` hop whose `didMigrate`
    ...
    /// hop and the additive `V4 → V5` title-bytes hop. A fresh store runs no
    /// stage and is created directly at V5; older
    ...
    internal static var stages: [MigrationStage] {
        [
            ...
            .lightweight(
                fromVersion: HistorySchemaV4.self,
                toVersion: HistorySchemaV5.self
            )
        ]
    }
}
