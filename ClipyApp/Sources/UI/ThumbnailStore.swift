...
final class ThumbnailStore {
    ...
    private(set) var debugFetchCompletionCount = 0
    private(set) var debugDiscardedFetchCompletionCount = 0
    #endif
    ...
    func imagePixelSize(for item: HistoryItemReference) -> PixelSize? {
    ...
    func prefetch(_ item: HistoryItemReference) {
    ...
    func discardColdPixelsForTesting(_ item: HistoryItemReference) {
        coldPixels.object(forKey: cacheKey(item))?.discardContentIfPossible()
    }

    func removeColdEntryForTesting(_ item: HistoryItemReference) {
        coldPixels.removeObject(forKey: cacheKey(item))
    }
    #endif
}
...
@MainActor
final class ThumbnailMeasurement {
