...
struct FilePreviewTests {
    @Test(arguments: ["file:///not-opened/document.pdf", "file:///not-opened/image.png"])
    func displayedFileReferencesDoNotReadTheirDestination(_ address: String) async throws {
        ...
        guard case .content(.reference(let reference)) = loader.phase else {
            Issue.record("Expected the copied reference")
            return
        }
