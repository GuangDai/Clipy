...
that violate the target's confinement rules:
  ...
  HistoryStorage     blocklist: AppKit, SwiftUI, PasteboardAdapter, PresentationUI
                     (SwiftData / ImageIO / xxh3 / HistoryCore / HistoryDomain allowed;
                     Fuse becomes an allowed HistoryStorage import at roadmap step 3
                     once pinned — no gate change needed here, HistoryStorage is
                     blocklist-based; consider a global "Fuse only in HistoryStorage"
                     rule when the dependency lands)
...
Global rule: ``import xxh3`` is forbidden outside HistoryStorage.
