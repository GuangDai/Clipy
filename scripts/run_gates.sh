...
#
#   1. scripts/tests/                  — diagnostic + evidence-workflow fixtures
#   2. scripts/evidence_workflow_gate.py — manual-only caller + liveness contract
...
echo "== gate 2/7: manual evidence workflow contract =="
if ! python3 "$REPO_ROOT/scripts/evidence_workflow_gate.py"; then
    status=1
fi
