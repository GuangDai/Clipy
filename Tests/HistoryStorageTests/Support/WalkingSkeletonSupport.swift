...
enum WSSupport {
    ...
    ///
    /// Built at the current immutable schema (`HistorySchemaV5`) since
    /// `SwiftDataHistory.open` constructs V5 stores through the migration
    ...
    /// writes, and every store it opens here is created at V5 (freshly by
