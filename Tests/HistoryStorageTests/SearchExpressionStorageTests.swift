...
struct SearchExpressionStorageTests {
    ...
    @Test func expressionPagesTraverseBothDirectionsAndBindTheWholeQuery() async throws {
        ...
        let secondNext: HistoryPageCursor = try #require(second.next)
        let third = try await search(query, in: history, limit: 2, cursor: secondNext)
        ...
        let thirdPrevious: HistoryPageCursor = try #require(third.previous)
        let backward = try await search(query, in: history, limit: 2, cursor: thirdPrevious)
    ...
    func emptyExpressionKeepsRecentOrderAndHasNoHighlights(query: String) async throws {
        ...
        let next: HistoryPageCursor = try #require(expression.next)
        let second = try await search(query, in: history, limit: 1, cursor: next)
