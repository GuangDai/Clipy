...
1. Reject a pasteboard-level exclusion (`ClipboardCapture.isConcealed` or any recognized transient/private/concealed/auto-generated sibling marker), an empty capture, a non-finite observation timestamp, or a hard-limit violation. Exclusion happens before payload validation or fingerprinting and returns `.invalidInput(.excludedFromHistory)`; a NaN/infinite `observedAt` returns `.invalidInput(.invalidTimestamp)` before fingerprinting.
...
3. Enforce whole-capture exclusion for the configured marker set; never filter a marker while retaining its sibling plaintext/rich representations. V1 recognizes `org.nspasteboard.TransientType`, `org.nspasteboard.ConcealedType`, `org.nspasteboard.AutoGeneratedType`, `com.agilebits.onepassword`, `de.petermaurer.TransientPasteboardType`, and `com.typeit4me.clipping`.
