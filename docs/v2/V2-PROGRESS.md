...
| 2026-08-23 | X.7 App Intents composition started. The current branch freezes the six-intent/one-provider design, identity-string reconstruction, output-only entity projection, real pasteboard write, and hosted-vs-signed evidence split. It is not a landed product capability; correctness and signed system-invocation evidence are pending. | X.7 | in progress | current worktree; no CI claim |
