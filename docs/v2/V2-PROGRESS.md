...
| 2026-08-23 | Batch 18 begins the unmerged X.9 F1 server-custody leaf. [`LocalAutomationCredential`](../../Sources/HistoryStorage/LocalAutomationCredential.swift) defines the exact UUID16 + secret32 value and the production Security random source; actor-confined [`CredentialStore`](../../Sources/HistoryStorage/CredentialStore.swift) selects an app-private Data Protection Keychain item. [`LocalAutomationCredentialStoreTests`](../../Tests/HistoryStorageTests/LocalAutomationCredentialStoreTests.swift) currently cover literal shape and the store semantics through an in-memory substitute for the Security boundary. No macOS correctness run has compiled this work, and the tests are not signed-artifact Keychain evidence. Client custody, enrollment/revocation coordination, authentication/comparison, ingress, Gateway/History dispatch, product CLI, and signed platform evidence remain open. | X.9 / F1 server custody | in progress | current worktree; CI pending |
| 2026-08-23 | Batch 18 adds a tests-only D1 retirement fixture at [`StableIdentityRetirementTests`](../../Tests/HistoryStorageTests/StableIdentityRetirementTests.swift): public History capture → remove → byte-identical recapture must mint a distinct item ID while the retired ID stays not-found. It changes no production source and has no macOS CI evidence yet. | D1 stable identity follow-up | in progress (tests only) | current worktree; CI pending |
| 2026-08-23 | Batch 18 adds the dedicated wrong-kind rate follow-up at [`GatewayConnectionKindRateDenialTests`](../../Tests/HistoryStorageTests/GatewayConnectionKindRateDenialTests.swift). The real in-memory V4 Authority seam covers bidirectional wrong-kind unaudited rejection and one correct-kind local rate-denial audit with History/HCR/position unchanged. This closes PR #19's behavior-fixture gap only after macOS correctness succeeds; it is not credential/ingress or signed evidence. | X.9 / expected-kind rate follow-up | in progress (tests only) | current worktree; CI pending |
| 2026-08-23 | Batch 18 adds a dispatch-only Card 5D Release-surface evidence leaf: [`release-surface.yml`](../../.github/workflows/release-surface.yml) invokes [`run_release_surface.sh`](../../scripts/ci/run_release_surface.sh) over one ordinary ad-hoc Release app and a finite reviewed [`release-forbidden-symbols.txt`](../../scripts/ci/release-forbidden-symbols.txt) inventory. No run exists yet. Its ceiling is exact absence of those fixed demangled literals in that artifact; it is not a complete instrumentation audit, distribution identity, notarization, Gatekeeper, TCC, UI, or runtime proof. | Card 5D Release symbol subleaf | in progress | workflow dispatch pending |
| 2026-08-23 | Batch 18 begins Card 5A capture-access wiring. [`PasteboardAccess`](../../Sources/PasteboardAdapter/PasteboardAccess.swift) translates documented AppKit access behavior into a neutral Sendable value; [`CaptureAccessState`](../../ClipyApp/Sources/Capture/CaptureAccessState.swift) owns the content-free reducer and only `.allowed` permits background polling. [`PasteboardObserver`](../../Sources/PasteboardAdapter/PasteboardObserver.swift) preflights access each immediate/timer cycle; [`PasteboardAccessTests`](../../Tests/PasteboardAdapterTests/PasteboardAccessTests.swift), [`AppCaptureAccessTests`](../../ClipyApp/Tests/ClipyIntegrationTests/AppCaptureAccessTests.swift), and [`PasteboardObserverRetryTests`](../../Tests/PasteboardAdapterTests/PasteboardObserverRetryTests.swift) cover the current mapping/reducer/wiring fixtures. CI is pending. No Pause UI or provider-timeout inference is added, and this cannot prove real General pasteboard prompts, TCC/System Settings behavior, clean-profile transitions, or signed runtime. | REVIEW Card 5A | in progress | current worktree; CI pending |
| 2026-08-23 | Batch 18 adds an optional hosted Card 5B characterization at [`AppCaptureExactOutcomeTests`](../../ClipyApp/Tests/ClipyIntegrationTests/AppCaptureExactOutcomeTests.swift). A named private pasteboard's real lazy-data provider declares one type but publishes no bytes; the test traverses the production observer/composition and real in-memory History, expecting one content-free declared-unavailable failure, zero bounded-lane occupancy, and unchanged empty History. The file is unmerged and CI-pending. It makes no provider-timeout/permission diagnosis and cannot prove General pasteboard or TCC behavior. | REVIEW Card 5B optional hosted evidence | in progress (tests only) | current worktree; CI pending |
