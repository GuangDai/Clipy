...
列表使用 SwiftUI 的行 ID 滚动锚点维持阅读位置；向任一方向追加页面并淘汰另一端页面，
不能把原有像素偏移直接套到新的三页窗口。接近视口边缘时按实际可见行预取相邻页，
只有将被淘汰的页面完全离屏才自动前进；异步读取结束时重新检查这一条件，以允许用户
在等待时反向滚动。加载范围与“较新／最新”导航持续可见，选择因窗口变化而调整时
不触发滚动，只有明确的键盘导航请求才滚动到选择项。
浮窗与设置中的历史工作区可分别选择打开最新记录或恢复阅读位置，默认仍是最新记录。
恢复只保存一条可见记录的 UUID，不保存内容、搜索词或游标。重新打开时按当前条件和
排序直接定位该条目，目标成为返回页的首行；条目已删除、被保留策略移除或不符合当前
条件时，重新读取当前查询首屏并显示说明。恢复请求不遍历前页来计算全局序号，因此
界面在抵达真正的首屏前显示“阅读位置附近”，计数保留为下限。读取恢复页后不再接受
位置较旧的缓冲首屏覆盖它；关闭或更换查询会取消尚未完成的恢复。
...
稀疏搜索候选需要额外排序时，排序记录携带正文的 rowid 和行元数据，而不是整段
`searchBodyUTF8`。只有排序后的行进入既有字节限额批次时，才在同一个只读快照内
按 rowid 读取正文；这增加一次索引定位，但避免临时排序器提前复制所有候选正文。

修订保留策略每批访问最多 32 个候选索引行，在 Swift 中判断标量阈值；没有命中的
页也推进游标并在准备阶段让出 Authority。只有实际超限的条目读取完整修订信息。
这里限制的是访问行数，而不只是 SQL `LIMIT` 返回的命中数。

HCR 保留清理只读取载荷长度和记录标量，完整编码校验仍属于启动读取。
Local Automation 的有效内容读取先根据同一事务中的元数据核对回复字节上限，
通过后才读取原始内容；拒绝和审计仍由既有 Gateway/Authority 原子操作处理。
