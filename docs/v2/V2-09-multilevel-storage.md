...
最新的多级存储方向设计终态；SQLite 元数据、不可变 blob 文件及派生缓存已替换
master 的 SwiftData/十模型内部持久化表达，逐表示消费等后续改造仍在进行。这一决定取代
...
UI 使用三页窗口；这些行为仍需规模测试验证。三页窗口
约束的是完整行 DTO；当前“Newer”仍保存已访问页的游标，数量随遍历增长，不能把
它描述成整个导航状态 O(1)。终态 recent 应提供同一快照上的双向 keyset，让窗口两端
各自能请求相邻页；搜索需要等价的有界结果定位。不要靠无限保留旧页书签实现倒翻。
