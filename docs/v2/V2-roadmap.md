...
- **Status:** X.1/X.2 contracts, X.3 schema/bootstrap, X.4 audit/admin,
  **X.5 internal denial, and X.6 positive Gateway/public facade are landed.**
  ...
  placement evidence. X.8 is the current implementation leaf.
...
| X.8 `clipyctl` pure codec — **current leaf** | Add the no-product, Foundation-only `ClipyCLIContract` target and freeze V2-05 §0.1.1–§0.1.2 as pure request decode, deterministic reply encode, and exit mapping. Protocol v1 admits only `browsePreview` with the exact recent/search argument union and bounds. Add no executable, `main`, `FileHandle`, process I/O, transport, credential, authenticated ingress, Gateway/History dependency, synthetic result, hash, digest, or idempotency state. Capability JSON remains an owner-summary declaration and is not an X.8 operation. | Independent `PLAY-PY-A2A` through `PLAY-PY-A2I` parser/renderer/closed-operation golden tests establish only the pure wire contract. They make no Python-to-History, positive Local Automation, transport, launch, authentication, or signed-runtime claim. |
| X.9 Authenticated ingress, one private transport, and the first read-only tracer | First run `PLAY-PY-F0A` as the evidence-only ad-hoc-signed UDS discriminator from V2-05 §0.4 and the [Apple/XNU memo §13.1](../reviews/2026-08-22-clipy-maccy-deep-review/apple-platform-source-memo.md): a `CLIPY_UDS_F0` main-app listener plus the XcodeGen `ClipyUDSF0Client` diagnostic tool exist only in the dispatched proof artifact and carry no X.8 JSON, credential, Gateway, History, or product `clipyctl` behavior. F0A may reject or justify further investigation of the UDS candidate; it does not select a production transport. Then resolve `DEC-PY-AUTHENTICATED-INGRESS`, credential custody, and target/access ownership before implementing one selected transport. X.9 eventually owns the first grant-gated `browsePreview` tracer, including warm B4 and cold B5; the same-EUID claim is account-wide, not login-session identity. | F0A must prove only cold, warm-same-generation/PID, and `SIGKILL` stale recovery with a new generation/PID in the exact ad-hoc-signed non-sandbox artifact. Different-UID and interactive no-activation cells remain open if the hosted runner cannot observe them. Product completion then requires `PLAY-PY-F1`, `PLAY-PY-B3`, `PLAY-PY-B3A`, `PLAY-PY-B3B`, `PLAY-PY-B3C`, `PLAY-PY-B4`, and `PLAY-PY-B5`. **BLOCKED-SPEC** after F0A while authenticated ingress and client credential custody remain unresolved. |
