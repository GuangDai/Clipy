...
| Finding | File:§ | Change |
|---|---|---|
| REVIEW unified-retention 尾巴裁决（doc 11 §4.1 L137）(2026-08-28) | V2-05 §2.2/§3.2/§0.2; V2-02 §9/§7; V2-07 §12; V2 roadmap X.7 | Task 47-4 route-B adjudication, no feature: retention-policy configuration has no external ingress. The frozen external face is the six-intent App Intents surface with the closed three-case `ExternalRequest` set and the 19-case `ExternalOperationKind` vocabulary — `ExternalHistory` cannot spell retention — and the local Settings path through the v1 `ClipboardHistory` seam (V2-02 §9) remains the sole retention owner. The internal `.setRetentionPolicies` sweep publishes no `OperationRecord`, and its whole-store destructive scope shares the exact reason V2-05 §2.2 excludes external `clear`; an external remove fires no retention expansion (V2-02 §7), so the Card 9B exact purge never misses a retention victim. Added hosted `ExternalRetentionIsolationTests`: after baking a non-default policy, exercising every constructible `ExternalRequest` case plus one read through the granted connection-bound facade leaves the persisted `retentionConfiguration()` bit-identical and admits only the closed external/admin audit vocabulary; extended `externalAccessVocabularyRejectsUnknownRawValues` so a raw 20+ `ExternalOperationKind` — any retention spelling — is red until a V2-05 amendment. doc 10 L318 / doc 11 L137 tails closed to match. No product source, Gateway operation, spec text, or external write path changed. |
