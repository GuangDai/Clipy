...
| `DEC-THUMB-CACHE` | 06 G1 + V2-04/V2-07 | **RESOLVED (2026-09-02)** — G1 双阈未达，不准入 shared completed cache，退回 v1 per-surface visible-state retention（裁决记录 V2-04 §2.4；V2-07 §5.4 与 06 §3 已同步） | — | 证据：PR #54 final correctness run 33224258855 + master run 33226454046（同产品码；`ThumbnailScrollMeasurementJourneyUITests` 两跑均绿，60 PNG/≥60 s 真实滚动）——decode 段 p95 ≈1.6 ms（<16 ms；fetch+decode 上界 p95 峰值 11.05 ms 仍 <16 ms）、identical completed requests 0%（<30%，repeatRequestRate 与 repeatDecodeCompletionRate 两读数均 0），两条件同败。后果：C1/C2 保持 design-only、不留 v1 surface；C2 的 G3 触发仍不可达；S1(G4)/C3(G6) 独立触发不受影响；reviews/05 §6 的 settled RSS 腿未测（双阈已同败，不改变结论）。 |
...
| `DEC-RET-AGE` | V2-02 + V2-07 | **RESOLVED** — event-triggered（V2-02 §2.2 L107 已记；2026-08-29 登记） | — | age 由事件触发评估（capture/policy set），无 wall-clock expiry 扫描；`RetentionSettingsDraft.ageEnforcementExplanation`（"Time passing alone doesn't remove items."）承载 UI 诚实条款。 |
| `DEC-CAPTURE-CLOCK` | 03a + V2-02 | **RESOLVED (DC-28, accepted 2026-08-15)** — admitted `observedAt` 为唯一时间事实（V2-02 §4.2 L567 已记；Batch 45/PR #50 `RetentionCaptureCompositionTests` 冻结证明；2026-08-29 登记） | — | rejected：Authority-admitted-time 权威与 finite-skew clamp；exposure 记入 V2-02 §8.3。 |
