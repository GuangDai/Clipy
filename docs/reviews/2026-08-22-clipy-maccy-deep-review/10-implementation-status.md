...
| Card 8G visible Clear/autocorrection | Partial | [`SearchHeaderView`](../../../Sources/PresentationUI/SearchHeaderView.swift)、[`clearSearch`](../../../Sources/PresentationUI/HistoryViewState.swift#L221) | view-state immediate recent test | stable AX IDs/autocorrection source已接线；headless CI不暴露SwiftUI AX controls，真实control/FKA仍Open |
...
| Card 8G search accessibility identity | In progress | `SearchHeaderView` | field/Clear获得stable AX ID且autocorrection关闭；真实AX control journey仍是runtime gate |
...
| Row accessibility actions | In progress | `HistoryRowView` | default Copy与Pin/Unpin/Details/Remove具名actions进入production source并由app CI编译；headless runner不暴露SwiftUI AX tree，runtime handler/FKA/label质量仍Open |
